...
and *why* each one matters. Written so I can defend every choice in an interview.
...
- **Passwords are hashed with bcrypt, never stored as plaintext.**
  ...
  sites, so a plaintext leak would compromise their *other* accounts too.
...
- **Password hashing is done asynchronously (`bcrypt.hash`), never the sync version.**
  ...
  main thread for that whole time, freezing the entire server for *every* user on
  *every* request. Async hands the work to the thread pool and keeps the server
...
- **Defense in depth: no single layer is "safe."**
  ...
  validation each close *some* holes, none closes all. Security is layered and
